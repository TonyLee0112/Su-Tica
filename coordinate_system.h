...
public:
	...
	coordinate(float a, float b, float c, int kind) {
		if (kind == 0) { x = a; y = b; z = c; rho = 0; phi = 0; r = 0; theta = 0; k = 0; }; // 직교좌표계
		if (kind == 1) { x = 0; y = 0; z = c; rho = a; phi = b; r = 0; theta = 0; k = 1; }; // 원통좌표계
		if (kind == 2) { x = 0; y = 0; z = 0; rho = 0; phi = c; r = a; theta = b; k = 2; }; // 구면좌표계
	}
	// member data
	float x, y, z;
	float rho, phi;
